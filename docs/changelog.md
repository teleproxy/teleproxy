...
- **Automatic ClientHello fragmentation against TSPU JA4 fingerprinting** ([#39](https://github.com/teleproxy/teleproxy/issues/39)). Russian DPI (TSPU/ASBI) classifies traffic to fake-TLS MTProxies by JA4-hashing the *client's* ClientHello — a fingerprint teleproxy itself can't change because the bytes are produced by the Telegram client. But teleproxy can change how those bytes hit the wire: the public listening socket now announces a 128-byte TCP MSS in the SYN-ACK, and the client kernel respects it, splitting the outgoing ClientHello (~500-700 bytes) across 4-5 TCP segments. Cipher list, extensions, signature algorithms and ALPN all end up straddling segment boundaries, so a single-packet JA4 extractor only sees a fragment of what it needs. The trick is automatic — no flag, no config, no upgrade-time decisions for the operator — and is scoped to the user-facing proxy port only; the HTTP `/stats` / `/metrics` listener keeps full system MSS. Direction matters here: TCP MSS is per-direction, and Telegram traffic is overwhelmingly download (server→client), so the constrained direction costs essentially nothing in throughput. Pairs with the existing [ServerHello segmentation](/features/dpi-resistance/#serverhello-response-variation) so *both* sides of the handshake become unfriendly to single-packet DPI. Genuinely novel — neither `mtg` nor `telemt` does this. See the new [Forced ClientHello fragmentation](/features/dpi-resistance/#forced-clienthello-fragmentation-automatic) section for the mechanism and rationale.
