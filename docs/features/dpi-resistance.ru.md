...
Teleproxy объявляет малый TCP MSS (256 байт) в SYN-ACK на публичном порту прокси. Ядро клиента подчиняется ограничению и режет исходящий ClientHello (~500-700 байт) на 2-3 TCP-сегмента. Список шифров, расширения, алгоритмы подписи и ALPN оказываются на стыке сегментов — DPI, который вычисляет JA4 по одному пакету, видит лишь фрагмент нужных ему данных.
...
Компромисс: Linux применяет MSS слушающего сокета к обоим направлениям каждого принятого соединения, поэтому сегменты сервер→клиент также ограничены 256 байтами. Количество пакетов вырастает примерно в 5 раз, а накладные расходы TCP/IP-заголовков растут с ~3% до ~15%. На современном оборудовании с TSO/GSO нагрузка на CPU остаётся управляемой, но прокси, упирающиеся в пропускную способность, увидят заметное ограничение. По умолчанию фрагментация включена — для большинства пользователей работающий, но медленный прокси предпочтительнее быстрого, но заблокированного.
